...
class Media {
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font, itemData }) {
    this.extra = 0
...
class App {
  ...
  onTouchMove(e) {
    ...
    this.dragDist = Math.abs(x - this.startX)
    const distance = (this.start - x) * (this.scrollSpeed * 0.025)
    this.scroll.target = this.scroll.position + distance
  }

  onTouchUp(e) {
    if (!this.isDown) return   // interaction didn't start on the canvas — ignore
    this.isDown = false
    if (this.dragDist < 5) {
      this._handleClick(e)
    }
  ...
  _handleClick(e) {
    ...
    // Deduplicate: only match the first visible occurrence (closest to center)
    let bestMedia = null
    let bestDist = Infinity
    for (const media of this.medias) {
      const pos = media.plane.position.x
      const halfW = media.plane.scale.x / 2
      if (worldX >= pos - halfW && worldX <= pos + halfW) {
    ...
    if (bestMedia) this.onItemClick(bestMedia.itemData)
  }
      ...
      {activeItem && (
        ...
        >
          <div className="cg-lightbox-inner" onClick={e => e.stopPropagation()}>
            ...
            <div className="cg-lightbox-img-wrap">
            ...
            <div className="cg-lightbox-body">
              <h3 className="cg-lightbox-title">{activeItem.text}</h3>

              {activeItem.details && Object.keys(activeItem.details).length > 0 && (
                <dl className="cg-lightbox-details">
                  {Object.entries(activeItem.details).map(([key, val]) => (
                    <div key={key} className="cg-lightbox-row">
                      <dt className="cg-lightbox-key">{key}</dt>
                      <dd className="cg-lightbox-val">{val}</dd>
                    </div>
                  ))}
                </dl>
              )}
